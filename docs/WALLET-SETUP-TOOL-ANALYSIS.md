...
| **Language** | User asks in Swedish; tool returns English. Agent might answer in English. | Low | Keep tool text in English; agent can translate. Optional: add one line "Answer in the user’s language if they asked in another language." in tool description for the agent. |
...
| **Agent misquotes** | Agent summarizes tool output and turns it into one wrong path. | Medium | Keep text clear and explicit: "The wallet is **wherever** wallet-rpc was started with --wallet-dir. It is **not** necessarily in the standard GUI paths (e.g. ~/Library/...)." Reduce over-simplification. |
